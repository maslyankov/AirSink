...
static std::string tap_socket_path = "";
...
static void print_info (char *name) {
    ...
    printf("-tap <sock> [AirSink] Stream raw H.264/H.265 NALUs to a Unix socket\n");
    printf("          for an external renderer. Use -vs 0 to skip the gst window.\n");
    ...
    for (int i = 1; i < argc; i++) {
	  ...
	  audio_rtp_pipeline.append(argv[++i]);
	    ...
	    int n;
            ...
            if (i >= argc - 1 || *argv[i+1] == '-') {
                fprintf(stderr, "-tap requires a Unix socket path argument\n");
                exit(1);
            }
            tap_socket_path = argv[++i];
        } else {
...
int main (int argc, char *argv[]) {
    ...
    if (!tap_socket_path.empty()) {
        if (frame_tap_init(tap_socket_path.c_str()) != 0) {
            fprintf(stderr, "frame_tap_init failed for %s — continuing without tap\n",
                    tap_socket_path.c_str());
        }
    }
