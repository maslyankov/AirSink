...
uint64_t video_renderer_render_buffer(unsigned char* data, int *data_len, int *nal_count, uint64_t *ntp_time) {
    ...
    if (data[0]) {
        ...
        /* AirSink frame tap: fan out the NALUs to a Unix socket so the
         * host app can decode/render them itself. When the tap is active we
         * SKIP the GStreamer push so uxplay doesn't double-decode and fight
         * for CPU/GPU with the host renderer. */
        {
            uint8_t tap_codec = !strcmp(renderer->codec, h265)
                ? FRAME_TAP_CODEC_H265
                : FRAME_TAP_CODEC_H264;
            frame_tap_write(tap_codec, (uint64_t) *ntp_time,
                            (const uint8_t *) data, (uint32_t) *data_len);
        }
        if (frame_tap_is_active()) {
            return 0;
        }
